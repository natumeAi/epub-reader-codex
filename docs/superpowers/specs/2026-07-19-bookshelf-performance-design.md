...
bootstrap 或 fallback 结果通过一个 reducer action 原子提交。patch 只有在 `incomingRevision === currentRevision + 1` 或响应明确声明当前 mutation 的 `baseRevision === currentRevision` 时才能应用。任何缺失实体、未知 key 或 revision 跳跃都会拒绝 patch 并触发恢复 bootstrap。
  ...
  "books": [
    {
      ...
      "title": "示例",
      "author": "作者",
      "coverUrl": "/covers/example.webp?v=content-hash",
...
旧 shelf、catalog、recent 路由在迁移周期内保持原有响应与错误语义。
...
纯排序成功时 `shelfKeys` 为 null，客户端保留已验证的乐观顺序。创建、归属移动和删除返回完整但精简的 `shelfKeys`，并 upsert 受影响书籍、文件夹和 preview IDs。删除影响最近阅读时返回新的最多 10 条 `recent`。
...
- 常用 statement 可按 database instance 缓存；测试注入的不同 database 不能共享 prepared statement。

不默认修改 SQLite durability pragma。只有基准证明 fsync 是剩余瓶颈且用户明确接受耐久性取舍时，才单独评估 `synchronous` 设置。

## Cover Delivery

封面采用内容版本 URL。版本来自封面文件内容 hash 或与内容一一对应的稳定版本值；mtime 只有在保证内容变化必然改变版本时才能使用。

- 版本化 URL 响应允许 `Cache-Control: private, max-age=31536000, immutable`。
- 封面内容变化必须先生成新文件/版本，再在同一 library mutation 中发布新 URL。
...
### Phase 3: Revision and compact mutations

增加 base revision、patch response、reader recent entry 和前台 revalidation。客户端停止成功 mutation 后的 `loadShelf()`。

### Phase 4: Sparse rank

根书架和文件夹内排序切换为 active/neighbor command 与单 rank 更新。旧全量排序路由继续存在但新客户端不调用。
